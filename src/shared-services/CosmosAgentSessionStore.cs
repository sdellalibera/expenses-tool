...
/// continuity across requests and server restarts. A new session document is created the first
/// time a conversation is seen, and the full message thread is saved on every turn.
/// </para>
///
/// <para><b>Container Requirements:</b></para>
/// <list type="bullet">
///   <item><description>Partition key: <c>/id</c></description></item>
...
public sealed class CosmosAgentSessionStore : AgentSessionStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var serializedSession = await agent
            .SerializeSessionAsync(session, jsonSerializerOptions: _serializationOptions, cancellationToken: cancellationToken)
            .ConfigureAwait(false);
