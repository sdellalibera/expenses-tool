...
| Chat returns HTTP 429 | Inspect `receipt.analyze` and the Agent Framework model spans to identify the failing stage. Honor the chat response's `Retry-After`, inspect errors and available usage attributes, then adjust the relevant budget or quota. The draft/photo are retained and completed receipt processing is reused on retry in the same conversation. |
